...
> ⚠ **2026-05-15 設計変更**：顧客分類を **5分類**に拡張しました。 // ← 変更
> - `routes/pricing.js` に `getCustomLookupKey(priceGroup, normId)` ヘルパーを追加
> - Group B → 疑似ID `GROUP_B` で Custom Prices を検索
> - Group C → 疑似ID `GROUP_C` で Custom Prices を検索
> - Individual → 従来通り Customer ID で検索
> - 詳細は `routes/pricing.js` の最新実装を参照（本ファイル中の旧コード例は3分類版なので注意）
...
[Step 3] 各SKUに対して価格決定
   ...
   │   ├ Standard → ベース価格
   │   ├ Group A  → ベース価格 × 1.020
   │   ├ Group B  → Custom Prices(GROUP_B + SKU) 検索 → 無ければベース価格 // ← 変更
   │   ├ Group C  → Custom Prices(GROUP_C + SKU) 検索 → 無ければベース価格 // ← 変更
...
const GROUP_A_MARKUP = 0.020; // 2.00%
...
- [ ] Group B顧客の注文（GROUP_B Custom Price あり）→ 共通Custom Price が反映される <!-- ← 変更 -->
- [ ] Group B顧客の注文（GROUP_B Custom Price なし）→ ベース価格にフォールバック <!-- ← 変更 -->
- [ ] Group C顧客の注文（GROUP_C Custom Price あり）→ 共通Custom Price が反映される <!-- ← 変更 -->
- [ ] Group C顧客の注文（GROUP_C Custom Price なし）→ ベース価格にフォールバック <!-- ← 変更 -->
