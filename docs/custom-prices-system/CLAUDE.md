...
- [x] **フェーズ2**: Client list W列・X列追加 ← commit 87398d0（スクリプト作成、列順W/X修正済）
  - [ ] **残作業**: 開発用 Client Information で `setup_price_group.js` を実行
  - [ ] **残作業**: 全顧客の Price Group分類（Group A=12社 / Individual=15社 / Standard残り）
- [x] **フェーズ3**: 初期データ移行 ← commit 6e6ca2c（調査完了・移行方針(C)手動Excel化に決定）
  ...
  - [ ] **残作業**: Individual 15社の Custom Price を開発用 Custom Prices シートに**手動入力**（≈75レコード）
