...
> **重要な設計変更（2026-05-22）**：顧客分類を**6分類**に拡張しました。 // ← 変更
> - 旧：Standard / Group A / Individual（3分類）→ Standard / Group A / Group B / Group C / Individual（5分類）
> - 新：Standard / Group A / Group B / Group C / **Group D** / Individual（**6分類**）
>   - Group B（Daikoku Group **6社**）：**6社で共通カスタム価格**（疑似ID `GROUP_B`）
>   - Group C（Manpuku **4社**）：**4社で共通カスタム価格**（疑似ID `GROUP_C`）
>   - Group D（Ramen Joint-Aikan **5社**）：**5社で共通カスタム価格**（疑似ID `GROUP_D`） ← 2026-05-22追加
> - 以降の各ドキュメントで「5分類」と書かれた箇所は「6分類（Group D含む）」と読み替えてください。Group D は Group B/C と同じ共通カスタム価格方式です。
