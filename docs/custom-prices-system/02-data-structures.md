...
> - **Custom Prices シート**は構造そのままで、**Customer ID 列に疑似ID（`GROUP_B` / `GROUP_C`）** を入れて Group B/C の共通価格を持たせます。
> - **Client list の W列プルダウン**は5択に拡張：`Standard / Group A / Group B / Group C / Individual`。
...
| シート名 | 場所 | 役割 |
|---|---|---|
| **Custom Prices** | Cost list | Individual顧客×SKUの個別単価 + Group B/C 共通単価マスター | // ← 変更
...
Group B / Group C / Individual 分類の顧客に対して設定された単価を管理するマスターシートです。 // ← 変更
- **Individual**：実顧客の Customer ID をキーに、各社個別の単価を持つ
- **Group B**：疑似Customer ID `GROUP_B` をキーに、Daikoku 6社共通の単価を持つ
- **Group C**：疑似Customer ID `GROUP_C` をキーに、Manpuku 4社共通の単価を持つ

### 2-2. 列構造

| 列 | ヘッダー | 型 | 必須 | 説明 |
|---|---|---|---|---|
| A | Customer ID | Text | ✅ | **3桁ゼロ埋め**（Individual）または **`GROUP_B` / `GROUP_C`**（共通価格）| // ← 変更
| B | Customer Name | Text | ✅ | 顧客名 / グループ名（参照用・表示用）|
| C | SKU | Text | ✅ | 商品コード（例：B033, P012）|
| D | Item Name | Text | ✅ | 商品名（参照用・表示用）|
| E | Custom Price | Number | ✅ | 個別/共通単価（USD、小数点以下2桁）|
| F | Update Date | Date | ✅ | 最終更新日（自動入力、形式：M/d/yyyy）|
| G | Note | Text | 任意 | メモ（例：「2026年5月見直し」「Daikoku 6社共通」）|

### 2-3. 重要なルール

1. **複合キー：Customer ID + SKU の組み合わせはユニーク**
   - 同じ顧客（または同じグループ疑似ID）に同じSKUで複数行は作らない
   - 重複追加時はエラーで防ぐ

2. **Customer ID の正規化**: // ← 変更
   - **Individual**：3桁ゼロ埋め（例：`11` ではなく `011`）。`normalizeId_()` で正規化
   - **Group B**：`GROUP_B` 固定（大文字、アンダースコア）
   - **Group C**：`GROUP_C` 固定
   - 疑似IDは `normalizeId_()` が「数字がない → そのまま返す」仕様なので変換されずに保持される
...
|---|---|---|---|---|---|---|
| GROUP_B | Group B (Daikoku - 6社共通) | B033 | Beef Short Rib | 21.80 | 5/15/2026 | Daikoku 6社共通 | <!-- ← 変更 -->
| GROUP_C | Group C (Manpuku - 4社共通) | B008 | Beef Ribeye 10oz | 14.50 | 5/15/2026 | Manpuku 4社共通 | <!-- ← 変更 -->
| 011 | XXX RESTAURANT | B033 | Beef Short Rib | 22.50 | 5/13/2026 | 初期移行 |
...
| 列 | ヘッダー | 型 | 必須 | 説明 |
|---|---|---|---|---|
| **W** | Price Group | Text | ✅ | `Standard` / `Group A` / `Group B` / `Group C` / `Individual` のいずれか | <!-- ← 変更 -->
| **X** | Markup % | Number | 条件付き | Group A のみ `2.0`、それ以外は空欄 |

> **注：** 仕様書初版では X・Y列とされていたが、実際の Client list は V列まで使用しているため、空き列を作らないよう **W・X列** に変更（2026-05-14）。

### 5-3. Price Group の値（5分類） <!-- ← 変更 -->

| 値 | 意味 | 該当顧客数 | Custom Prices |
|---|---|---|---|
| **Standard** | 標準価格をそのまま適用 | 約30社 | 使わない |
| **Group A** | スタンダード価格 + 2.00% | **12社**（Jinya Group） | 使わない |
| **Group B** | Daikoku 6社共通カスタム価格 | **6社** | `GROUP_B` 疑似IDで検索、なければ Standard |
| **Group C** | Manpuku 4社共通カスタム価格 | **4社** | `GROUP_C` 疑似IDで検索、なければ Standard |
| **Individual** | 各社個別カスタム価格 | **9社** | Customer ID で検索、なければ Standard |
...
```
入力候補: Standard, Group A, Group B, Group C, Individual  <!-- ← 変更（5択） -->
他の値は拒否
```

### 5-5. X列（Markup %）の補足

- Group A の場合：`2.0` を入力（実際の計算では `1 + 2.0/100 = 1.020` を掛ける）
- **Group B / Group C / Individual / Standard**：空欄（一律％は使わない） <!-- ← 変更 -->
- **このX列は Group A 専用**で、その他の分類では参照されない
- 将来「Group D = +5%」など一律％分類を追加する際に活用
...
### 5-7. 初期データ投入方針 <!-- ← 変更（5分類対応） -->

W列に値を入れる方法（推奨順）：

1. **Group A の12社を特定** → 該当行のW列に `Group A` を入力、X列に `2.00`
2. **Group B の6社を特定**（Daikoku Group） → 該当行のW列に `Group B` を入力
3. **Group C の4社を特定**（Manpuku） → 該当行のW列に `Group C` を入力
4. **Individual の9社を特定** → 該当行のW列に `Individual` を入力
5. **残りすべて** → W列に `Standard` を一括入力

具体的な顧客リストは Manabu さんの判断で投入します。詳細手順は **手順書2** 参照。
