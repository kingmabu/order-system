...
| **X** | Markup % | Number | 条件付き | Group Aの場合：`2.0`、それ以外は空欄 |
...
| **Group A** | スタンダード価格 + 2.00% | 12社（Jinya Group） |
...
- それ以外：空欄
- **このX列は将来の拡張用**で、現状のロジックでは使わない（Group A = +2.00%固定）
...
1. **Group A の12社を特定** → 該当行のW列に `Group A` を入力
